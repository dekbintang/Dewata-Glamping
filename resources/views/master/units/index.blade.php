...
@section('title', 'Unit Glamping')
@section('header', 'Unit Glamping')

@section('content')
    <div class="flex items-center justify-between mb-5">
        <div>
            <h3 class="text-lg font-semibold text-gray-800">Daftar Unit Glamping</h3>
            <p class="text-sm text-gray-400">Kelola data unit akomodasi glamping Anda.</p>
        </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <x-data-table :headers="['#', 'Nama Unit', 'Tipe', 'Kapasitas', 'Harga Dasar', 'Status', 'Aksi']">
            @foreach($units as $i => $unit)
                <tr class="hover:bg-gray-50 transition-colors">
                    ...
                    <td class="px-5 py-3 text-gray-600">{{ $unit->capacity }} orang</td>
                    <td class="px-5 py-3 font-medium text-gray-800">Rp {{ number_format($unit->base_price, 0, ',', '.') }}</td>
                    <td class="px-5 py-3"><x-status-badge :status="$unit->status" /></td>
                    <td class="px-5 py-3">
